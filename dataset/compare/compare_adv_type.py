"""
goyo_extracted（NAIST誤用コーパス）と ADV_v1.tsv の
副詞誤り(ADV)データを比較する統計分析スクリプト
"""
import sys, csv, statistics, re
import openpyxl
...
def load_artificial_adv(tsv_path):
    records = []
...
def classify_adv(err_tok, cor_tok):
    e = str(err_tok) if err_tok else ''
    c = str(cor_tok) if cor_tok else ''

    # 1. 〜りに → 〜りと（オノマトペ副詞の格助詞誤り）
    if e.endswith('りに') and c.endswith('りと'):
        return '〜りに→りと（助詞誤り）'

    # 2. ナ形容詞+と → に（形容動詞副詞化誤り）
    if e.endswith('と') and c.endswith('に'):
        return 'ナ形容詞+と→に（副詞化誤り）'

    # 3. 形容詞→副詞化（〜い → 〜く）
    if e.endswith('い') and c.endswith('く'):
        return '形容詞→副詞化（〜い→く）'

    # 4. 語順誤り（動詞の後に副詞）
    if re.search(r'(ます|ません|した|している|してい|ください)(?!。?$)', e):
        return '副詞語順誤り'

    # 5. 副詞過剰使用・重複（cor_tok が err_tok に含まれる）
    if c and c in e and len(e) > len(c) + 1:
        return '副詞過剰使用・重複'
...
def print_subtype_compare(goyo_recs, art_recs):
    def count_subtypes(recs):
        counts = {}
        for r in recs:
            st = classify_adv(r['err_tok'], r['cor_tok'])
...
def main():
    ...
    print("データ読み込み中...")
    goyo_recs = load_goyo_adv(goyo_path)
    art_recs  = load_artificial_adv(tsv_path)

    if not goyo_recs:
        print("ERROR: goyo_extracted に ADV型データが見つかりません")
        print("  → typeカラムが 'adv' のデータが存在するか確認してください")
        return
    if not art_recs:
        print("ERROR: ADV_v1.tsv に ADV型データが見つかりません")
        return
    ...
    # ---- goyo ----
    print(f"\n{SEP}")
    print(f" NAIST誤用コーパス（goyo_extracted）: ADV型  {gs_g['n']}件")
    print(SEP)
    print_stats(gs_g, '誤文')
    print_stats(gs_s, '正文')
    ...
    # ---- 人工データ ----
    print(f"\n{SEP}")
    print(f" 人工データ（ADV_v1.tsv）: ADV型  {as_g['n']}件")
    print(SEP)
    print_stats(as_g, '誤文')
    print_stats(as_s, '正文')
    ...
    for st, cnt in sorted(counts.items(), key=lambda x: -x[1]):
        ...
        print(f"  {st:<34}: {cnt:3d}件 ({cnt/total*100:4.1f}%)  No.{sample}{ellip}")
